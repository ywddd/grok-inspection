...
- 可选自动定时巡检（cron，默认关闭）；定时路径可配置自动删除权限被拒、禁用额度用尽、启用健康已禁用
...
| 项 | 说明 |
|----|------|
| 启用定时 | 默认关闭；开启后按 cron（进程本地时区）触发 |
| Cron | 5 字段表达式，默认 `0 3 * * *`（每天 3:00） |
| 定时并发 | 定时完整巡检的 workers（1–16，默认 6） |
| 自动删除权限被拒 | 仅定时整轮结束后，删除本轮 `permission_denied` |
| 自动禁用额度用尽 | 仅定时整轮结束后，禁用本轮 `quota_exhausted` 且未禁用 |
| 自动启用健康已禁用 | 仅定时整轮结束后，启用本轮 `healthy` 且已禁用 |

约定：

- 定时始终跑**完整巡检**，并**包含已禁用**账号。
- 自动处置**只在定时路径**生效；手动完整/增量/分类巡检不会自动改账号。
- 自动删除/禁用/启用依赖 CPA 进程环境变量 `MANAGEMENT_PASSWORD` 或 `CPA_MANAGEMENT_KEY`（不落盘页面 Key）。
- 到点若正在巡检或批量操作，本轮**跳过**，等下次 cron。
- 配置保存在 `data/grok-inspection/schedule.json`。
...
- 定时配置保存在同目录 `schedule.json`
- 结果文件只保存展示所需信息，不保存完整 token
- 默认关闭自动处置；仅开启定时自动开关后，才会在**定时巡检**结束后自动删/禁/启
- 手动路径的禁用/删除仍需你在页面确认后执行
